...
//Ваша задача — найти первый элемент массива, который не является следующим по порядку (то есть нарушает последовательность).
//Под нарушением последовательности понимается ситуация, когда элемент не больше предыдущего ровно на 1.
//Например, в массиве `[1, 2, 3, 4, 6, 7, 8]` элементы 1, 2, 3 и 4 идут последовательно, а число 6 — нет; следовательно, 6 — это первое число, нарушающее последовательность.
//Если все элементы массива идут последовательно, верните `null`.
//Массив всегда будет содержать как минимум 2 элемента, и все они будут числами. Числа будут уникальными и отсортированными по возрастанию. Они могут быть как положительными, так и отрицательными; число, нарушающее последовательность, также может быть любым из них.
...
//}
//
//let userPeopleAndWoman = PeopleWoman(nameWoman: "Katya", people: "Maks")
//print(userPeopleAndWoman.name)
//print(userPeopleAndWoman.nameWoman)




//class Button {
//    var title: String = "Access"
//}
//
//
...
//}
//
//let result = UIController()
//print(result.titleController)
//print(result.title)
...
// ============================================================================
// MARK: - ЛЕГКИЙ УРОВЕНЬ (1-7)
// ============================================================================

// ----------------------------------------------------------------------------
// 1. Создание переменных
// ----------------------------------------------------------------------------
// Создайте константу `name` с вашим именем и переменную `age` с вашим возрастом.
// Затем создайте строку `description` через интерполяцию:
...
// 2. Условный оператор
// ----------------------------------------------------------------------------
// Напишите функцию `canBuyAlcohol(age: Int) -> Bool`, которая возвращает `true`,
...
// 3. Массивы
// ----------------------------------------------------------------------------
// Дан массив `[10, 20, 30, 40, 50]`. Напишите код, который:
// - Добавляет число 60 в конец
// - Удаляет первый элемент
// - Выводит сумму всех элементов через `reduce`
// ----------------------------------------------------------------------------
// РЕШЕНИЕ:
...
// 4. Строки
// ----------------------------------------------------------------------------
// Напишите функцию `isPalindrome(_ text: String) -> Bool`, которая проверяет,
// является ли строка палиндромом (читается одинаково слева направо и справа налево).
// Например, "казак" → true, "привет" → false.
// ----------------------------------------------------------------------------
// РЕШЕНИЕ:
...
// 5. Опционалы
// ----------------------------------------------------------------------------
// Создайте опциональную переменную `middleName: String? = nil`.
// Используя `??`, присвойте значение "Не указано" новой константе `displayName`.
// ----------------------------------------------------------------------------
// РЕШЕНИЕ:
...
// 6. Множества (Set)
// ----------------------------------------------------------------------------
// Даны два массива: `[1, 2, 3, 4, 5]` и `[4, 5, 6, 7, 8]`.
// Найдите:
...
// 7. Словари
// ----------------------------------------------------------------------------
// Создайте словарь `[String: Int]` с оценками студентов:
// ["Анна": 5, "Петр": 4, "Мария": 3].
// Добавьте нового студента "Иван" с оценкой 4,
// затем измените оценку Марии на 5,
// и выведите всех студентов в цикле.
// ----------------------------------------------------------------------------
// РЕШЕНИЕ:
...
// ============================================================================
// MARK: - СРЕДНИЙ УРОВЕНЬ (8-14)
// ============================================================================

// ----------------------------------------------------------------------------
// 8. Функции с кортежами
// ----------------------------------------------------------------------------
// Напишите функцию `minMaxAverage(_ numbers: [Int]) -> (min: Int, max: Int, average: Double)?`,
// которая возвращает кортеж с минимальным, максимальным и средним значением.
// Если массив пуст, верните `nil`.
// ----------------------------------------------------------------------------
// РЕШЕНИЕ:
...
// 9. Switch + enum
// ----------------------------------------------------------------------------
// Создайте `enum Season { case winter, spring, summer, autumn }`.
// Напишите функцию `getMonths(season: Season) -> [String]`, которая возвращает
// массив месяцев для каждого сезона.
// Например: .winter → ["Декабрь", "Январь", "Февраль"]
// ----------------------------------------------------------------------------
// РЕШЕНИЕ:
...
// ----------------------------------------------------------------------------
// 10. Классы и наследование
// ----------------------------------------------------------------------------
// Создайте класс `Animal` со свойством `name: String` и методом `makeSound() -> String`.
// Создайте классы `Dog` и `Cat`, наследующие от `Animal`.
// Переопределите метод `makeSound()`:
// - собака → "Гав!"
// - кот → "Мяу!"
// ----------------------------------------------------------------------------
// РЕШЕНИЕ:
...
// Используя `filter`, оставьте только четные числа.
// Используя `map`, умножьте все оставшиеся числа на 3.
// Используя `reduce`, найдите сумму результата.
// ----------------------------------------------------------------------------
// РЕШЕНИЕ:
...
// ----------------------------------------------------------------------------
// 12. Структуры vs Классы
// ----------------------------------------------------------------------------
// Создайте структуру `Point` с координатами `x: Double, y: Double`
// и методом `distance(to point: Point) -> Double`.
// Создайте два экземпляра и вычислите расстояние между ними.
...
// ----------------------------------------------------------------------------
// 13. Guard и ранний выход
// ----------------------------------------------------------------------------
// Напишите функцию `processUser(name: String?, age: Int?) -> String`,
// которая использует `guard` для проверки:
// - Если `name == nil` → вернуть "Имя не указано"
// - Если `age == nil` → вернуть "Возраст не указан"
// - Если всё есть → вернуть "Привет, {name}! Тебе {age} лет."
// ----------------------------------------------------------------------------
// РЕШЕНИЕ:
...
// ----------------------------------------------------------------------------
// 14. Массивы + замыкания
// ----------------------------------------------------------------------------
// Дан массив строк: `["apple", "banana", "cherry", "date", "elderberry"]`.
// Используя методы высшего порядка:
// - Отфильтруйте строки длиннее 5 символов
// - Отсортируйте их по убыванию длины
// - Объедините в одну строку через запятую
// ----------------------------------------------------------------------------
// РЕШЕНИЕ:
...
// ============================================================================
// MARK: - СЛОЖНЫЙ УРОВЕНЬ (15-20)
// ============================================================================

// ----------------------------------------------------------------------------
// 15. Перечисления с ассоциированными значениями
// ----------------------------------------------------------------------------
// Создайте `enum CalculationResult` с кейсами:
// - `.success(value: Double)`
// - `.error(message: String)`
// Напишите функцию `divide(a: Double, b: Double) -> CalculationResult`,
// которая возвращает `.success` при делении и `.error` при делении на ноль.
// ----------------------------------------------------------------------------
// РЕШЕНИЕ:
...
//}
//
//func divide(a: Double, b: Double) -> CalculationResult {
//    if b == 0 {
//        return .error(message: "Can't divide by zero")
//    } else {
...
//Рассмотрим треугольник, составленный из последовательных нечетных чисел:
//
//             1
//          3     5
//       7     9    11
...
//21    23    25    27    29
//...
//Вычислите сумму чисел в n-й строке этого треугольника (нумерация строк начинается с 1), например: (Входные данные --> Результат)
//
//1 -->  1
//2 ​​--> 3 + 5 = 8
...
//Вы только что поселились на идеально прямой улице, где по обе стороны дороги расположено ровно по $n$ одинаковых домов. Разумеется, вам хочется узнать номер дома, в котором живут люди на противоположной стороне. Улица выглядит примерно так:
//
//Улица
//1|   |6
//3|   |4
//5|   |2
//  вы
//Четные номера возрастают справа; нечетные убывают слева. Нумерация домов начинается с 1 и идет последовательно, без пропусков. При $n = 3$ дом 1 находится напротив дома 6, дом 3 — напротив дома 4, а дом 5 — напротив дома 2.
//
//Пример (адрес, $n$ --> результат)
//Зная номер вашего дома (адрес) и количество домов на одной стороне улицы ($n$), определите номер дома на противоположной стороне.
//
//1, 3 --> 6
//3, 3 --> 4
//2, 3 --> 5
...
//Если возникают проблемы с превышением времени выполнения (timeout), нехваткой памяти или любые другие ошибки, прочитайте следующее. Значения $n$ и адреса могут достигать 500 миллиардов, а количество случайных тестов превышает 200. Если вы попытаетесь сохранить адреса 500 миллиардов домов в списке, память закончится, и тесты завершатся с ошибкой. Это не проблема самой задачи (ката), поэтому, пожалуйста, не создавайте тикеты об ошибках. Аналогично, если тесты не выполняются за 12 секунд, вы также не пройдете проверку.
//
//Чтобы решить эту задачу, нужно найти способ выполнить её без создания огромных списков или массивных циклов. Почитайте обсуждение (discourse), чтобы найти идеи :)
...
let res = overTheRoad(address: 3, street: 50)
print(res)
...
//
//Примечания
//Все входные данные представляют собой корректные треугольники с целочисленными сторонами.
//Округление не требуется. Правильность ответов будет проверяться с допустимой погрешностью 0,01.
...
let resHeron = heron(3, 4, 5)
print("resHeron -> \(resHeron)")
